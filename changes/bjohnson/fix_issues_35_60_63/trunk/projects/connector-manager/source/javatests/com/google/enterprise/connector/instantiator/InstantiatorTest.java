...
public class InstantiatorTest extends TestCase {
  ...
  public final void testAddUpdateDelete() throws JSONException,
    ...
    /**
     * Test dropping connectors.  Once dropped, I should not be able to fetch
     * get items from its interface.
     */
    instantiator.dropConnector("connector1");
    try {
        AuthorizationManager authz = instantiator.getAuthorizationManager("connector1");
    ...
    try {
        AuthenticationManager authn = instantiator.getAuthenticationManager("connector2");
  ...
  private class Issue63ChildThread extends Thread {
      ...
      public void run() {
        try {
          ...
          // Sleep for 10 seconds, allowing the test thread time 
          // to update the connector.
          try { Thread.sleep(10 * 1000); }
          catch (InterruptedException ie) {}

          // Get the Traverser for our connector instance.
          // It should be a new taverser reflecting the updated connector.
          newTraverser = instantiator.getTraverser("connector1");
  ...
  private int connectorCount()
      throws JSONException, InstantiatorException {
  ...
  private boolean connectorExists(String connectorName)
      ...
      Iterator iter = instantiator.getConnectorNames();
      while (iter.hasNext()) {
          if (connectorName.equals((String) iter.next()))
              return true;
      }
      return false;
  }
