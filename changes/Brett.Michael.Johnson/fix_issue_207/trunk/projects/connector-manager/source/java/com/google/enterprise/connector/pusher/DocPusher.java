...
public class DocPusher implements Pusher {
  ...
  /**
   * Allocates initial memory for a new XmlFeed and feed logger while trying
   * not to run out of memory.  If the process is low on memory and there are
   * outstanding submitted feeds, then wait until they clear before allocating
   * the buffers.
   *
   * @param feedType
   */
  private void startNewFeed(String feedType)
      throws PushException, FeedException, RepositoryException {
    int feedSize = (int) fileSizeLimit.maxFeedSize();
    // TODO: Perhaps this is too much effort here.  Rather than waiting around
    // trying to allocate a big buffer, I could allocate a tiny buffer (1KB?).
    // This would let this 1 document get fed, then take() would return false,
    // signalling the caller to back off.
    do {
      // Try for 5 minutes to allocate memory.
      for (int i = 0; i < 30; i++) {
        if (!lowMemory()) {
        ...
        // We are either very low on memory, or failed to allocate some.
        // Wait for 10 seconds to see if memory becomes available.
        try {
          Thread.sleep(10000);
          int j = checkSubmissions();
        } catch (InterruptedException ie) {
          throw new PushException("Pusher thread was interrupted before "
                                  + "document could be fed.");
        }
      }

      // Unable to allocate a feed buffer after 5 minutes of trying.
      // Reduce the requested feed size and try again.
      feedSize /= 2;
    } while (feedSize >= 1024 * 1024);
    throw new OutOfMemoryError("Unable to allocate feed buffer for connector "
                               + connectorName);
  }
