...
public class SetManagerConfigHandler {
  ...
  public SetManagerConfigHandler(Manager manager, String xmlBody) {
    ...
    Map currentSettings = manager.getConnectorManagerConfig();
    String currentFeedHost =
        (String) currentSettings.get(ServletUtil.XMLTAG_FEEDERGATE_HOST);
    int currentFeedPort = Integer.parseInt(
        (String) currentSettings.get(ServletUtil.XMLTAG_FEEDERGATE_PORT)); 
    if (currentFeedHost.equals(feederGateHost) &&
        currentFeedPort == feederGatePort) {
