...
public class QueryTraversalUtil {
	...
	public static int runTraversal(QueryTraversalManager queryTraversalManager,
			int batchHint, HashMap map) throws RepositoryException {
		...
		while (true) {
			...
			for (Iterator iter = resultSet.iterator(); iter.hasNext();) {
				...
				if (contentUrlProp != null) {
					...
					// if there is no contentUrl, the connector manager will ask
					// for
					// the content property and will base-64 encode it. Here we
					// will
					// only access the first so many characers of the content
					// and
					// print it out
					...
					if (contentProp != null) {
						...
						if (contentType == ValueType.STRING) {
							...
							if (fullContent == null) {
								...
								contentSnippet = fullContent.substring(0,
										snippetLength);
							} else {
...
*/				addOne(map, name);
	...
	static void addOne(Map map, String docId) {

		int itemIndex = docId.indexOf("}");
		String listKey = docId.substring(0, itemIndex + 1);
		Object entry = map.get(listKey);
		if (entry != null)
		{
			int count = ((Integer)entry).intValue();
			count++;
			map.put(listKey, new Integer(count));
		}else
		{
			map.put(listKey, new Integer(1));
		}
	}

	static PropertyMap processResultSet(ResultSet rs, int size, Map map)
			throws Exception {
		...
		while (it.hasNext()) {
			...
			Property contentUrlProp = pm
					.getProperty(SpiConstants.PROPNAME_CONTENTURL);
			if (contentUrlProp != null) {
				String contentSnippet = contentUrlProp.getValue().getString();
				System.out.println("Content");
				System.out.println(contentSnippet);
			}
			Property nameProp = pm.getProperty(SpiConstants.PROPNAME_DOCID);
